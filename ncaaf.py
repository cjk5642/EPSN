...
class NCAAF:
    ...
    for team in tqdm(teams):
      ...
      for game in games:
        ...
        edge = {'source': away_team, 'target': home_team, 'source_win': away_win, 
                'target_win': home_win, "source_rank": away_team_rank, "target_rank": home_team_rank,
                'source_conference': away_team_conf, 'target_conference': home_team_conf,
                'week': game.game, "conference_game": conf_game, 'year': self.year}
        edges.append(edge)
  ...
  def _prepare_player_data(self):
    ...
    for team in tqdm(teams):
      ...
      team_sched = ncaaf_schedule(team_abbr, year = self.year)
      for game in team_sched._games:
        ...
        for away_player, home_player in tqdm(source_to_target_players):
          ...
          if nodes.get(away_player_id) is None:
            try:
              away_player_position = ncaaf_player(away_player_id).position
            except TypeError:
          ...
          if nodes.get(home_player_id) is None:
            try:
              home_player_position = ncaaf_player(home_player_id).position
            except TypeError:
              away_player_position = ""
